...
{\fontsize{24pt}{24pt}\selectfont 帯電ダストダイナミクス解析のための}\\[0.5cm]
{\fontsize{24pt}{24pt}\selectfont 個別要素法シミュレーションモデルの開発}
...
\subsection{検証結果}
Fig.~\ref{fig:damped_oscillator}に、数値解析結果と理論解（式\ref{eq:damped_solution}）の比較を示す。
粒子の変位および速度の時刻歴は理論解と極めて良好に一致しており、法線方向の接触力計算およびLeapfrog法による時間積分が正しく実装されていることを確認した。さらに、数値解から算出した反発係数は設定値（$e=0.8$）と相対誤差1\%以内で一致した。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/damped_oscillator.png}
    \caption{Comparison of displacement and velocity time histories in damped oscillation. Solid lines represent numerical solutions, dashed lines represent theoretical solutions.}
...
Fig.~\ref{fig:wall_collision}に、衝突過程における粒子の重なり量の時刻歴を示す。
...
\begin{figure}[htbp]
    ...
    \caption{Time history of overlap and velocity in particle-wall collision. Top: overlap, Bottom: relative velocity. Solid lines represent numerical solutions, dashed lines represent theoretical solutions.}
...
Fig.~\ref{fig:particle_collision}に、衝突前後の2粒子の速度変化を示す。
...
\begin{figure}[htbp]
    ...
    \caption{Verification of velocity change in head-on collision between two particles. The characteristic of elastic collision where velocities are exchanged after collision is correctly reproduced.}
...
Fig.~\ref{fig:slope_velocity}に、斜面上の粒子の並進速度の時刻歴を示す。
...
Fig.~\ref{fig:slope_velocity_rolling}に、転がり条件における斜面上の粒子の並進速度の時刻歴を示す。
数値解析結果は理論加速度と良好に一致し、転がり条件が正しく実現されていることを確認した。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/slope_velocity.png}
    \caption{Time history of particle translational velocity in slope sliding test. Under sliding condition ($\mu=0.2$), numerical solution shows good agreement with theoretical acceleration $a = g(\sin\theta - \mu\cos\theta)$.}
    \label{fig:slope_velocity}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/slope_velocity_rolling.png}
    \caption{Time history of particle translational velocity in slope rolling test. Under rolling condition ($\mu=0.5$), numerical solution shows good agreement with theoretical acceleration $a = \frac{5}{7}g\sin\theta$.}
    \label{fig:slope_velocity_rolling}
\end{figure}
...
\subsection{検証結果}
Fig.~\ref{fig:coulomb_valid}に、数値解と理論解（式\ref{eq:coulomb_time_distance}の逆関数）の比較を示す。
...
\begin{figure}[htbp]
    ...
    \caption{Verification of Coulomb interaction between two particles. Top: inter-particle distance, Middle: particle velocity, Bottom: Coulomb force. Solid lines represent numerical solutions, dashed lines represent theoretical solutions.}
    \label{fig:coulomb_valid}
\end{figure}

\section{本章のまとめ}
以上の検証計算により、本研究で開発したDEMコードにおける各物理モデルが理論解と良好に一致することを確認した。
Table~\ref{tab:validation_summary}に検証結果の概要を示す。

\begin{table}[htbp]
\centering
\caption{Summary of validation calculations and results}
\label{tab:validation_summary}
\begin{tabular}{lll}
\hline
Validation Item & Validation Content & Relative Error \\
\hline
Damped oscillation & Time integration / damping model & $< 1\%$ \\
Particle-wall collision & Hertz contact / max. overlap & $< 0.1\%$ \\
Particle-particle collision & Momentum / energy conservation & $< 10^{-10}$ \\
Slope sliding & Sliding / rolling friction model & $< 1\%$ \\
Coulomb interaction & Electrostatic force calculation & $< 10^{-6}$ \\
\hline
\end{tabular}
\end{table}
...
Fig.~\ref{fig:wall_withdrawal}に、壁引き抜き法によるシミュレーションの様子を示す。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{Before wall withdrawal: particles filled in rectangular container}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{After wall withdrawal: slope formed by particle collapse}
    \end{subfigure}
    \caption{Simulation of angle of repose measurement using wall withdrawal method. Top shows particle arrangement before wall withdrawal, bottom shows after.}
...
安息角シミュレーションに用いた計算条件をTable~\ref{tab:repose_params}に示す。

\begin{table}[htbp]
\centering
\caption{Simulation conditions for angle of repose}
\label{tab:repose_params}
\begin{tabular}{lll}
\toprule
Parameter & Value & Unit \\
\midrule
Particle radius $r$ & 1.0 & mm \\
Particle density $\rho$ & 2480 & kg/m$^3$ \\
Young's modulus (particle) $E_p$ & $4.9 \times 10^9$ & Pa \\
Young's modulus (wall) $E_w$ & $3.9 \times 10^9$ & Pa \\
Poisson's ratio (particle) $\nu_p$ & 0.23 & - \\
Poisson's ratio (wall) $\nu_w$ & 0.25 & - \\
Friction coeff. (particle-particle) $\mu_p$ & 0.0 $\sim$ 1.0 & - \\
Friction coeff. (wall-particle) $\mu_w$ & 0.17 & - \\
Container width $W$ & 1.5 & m \\
Time step $\Delta t$ & $10^{-7}$ & s \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:friction_results}に結果を示す。

\begin{table}[htbp]
\centering
\caption{Relationship between sliding friction coefficient and angle of repose}
\label{tab:friction_results}
\begin{tabular}{ccc}
\toprule
Sliding friction coeff. $\mu$ [-] & Angle of repose $\theta_r$ [deg] & Coeff. of determination $R^2$ \\
\midrule
0.0 & 0.93 & 0.74 \\
0.2 & 9.64 & 0.92 \\
0.4 & 11.47 & 0.88 \\
...
Fig.~\ref{fig:friction_vs_angle}に、滑り摩擦係数と安息角の関係を示す。
...
\begin{figure}[htbp]
    ...
    \caption{Relationship between sliding friction coefficient and angle of repose. Angle of repose increases with friction coefficient but shows saturation tendency for $\mu > 0.4$.}
...
Table~\ref{tab:rolling_friction_results}に結果を示す。

\begin{table}[htbp]
\centering
\caption{Relationship between rolling friction coefficient and angle of repose}
\label{tab:rolling_friction_results}
\begin{tabular}{ccc}
\toprule
Rolling friction coeff. $\mu_r$ [-] & Angle of repose $\theta_r$ [deg] & Coeff. of determination $R^2$ \\
\midrule
0.0 & 1.51 & 0.50 \\
0.2 & 8.24 & 0.92 \\
0.4 & 8.90 & 0.90 \\
...
Fig.~\ref{fig:rolling_friction_vs_angle}に、転がり摩擦係数と安息角の関係を示す。
...
\begin{figure}[htbp]
    ...
    \caption{Relationship between rolling friction coefficient and angle of repose. Introduction of rolling friction significantly increases the angle of repose.}
...
Table~\ref{tab:all_params}に、DEMプログラムで使用可能なすべての入力パラメータを示す。

\begin{table}[htbp]
\centering
\caption{List of input parameters}
...
\midrule
\texttt{TIME\_STEP} & Time step & s \\
\texttt{MAX\_CALCULATION\_STEPS} & Maximum calculation steps & - \\
\texttt{YOUNG\_MODULUS\_PARTICLE} & Young's modulus of particle & Pa \\
\texttt{YOUNG\_MODULUS\_WALL} & Young's modulus of wall & Pa \\
\texttt{POISSON\_RATIO\_PARTICLE} & Poisson's ratio of particle & - \\
\texttt{POISSON\_RATIO\_WALL} & Poisson's ratio of wall & - \\
\texttt{PARTICLE\_DENSITY} & Particle density & kg/m$^3$ \\
\texttt{FRICTION\_COEFF\_PARTICLE} & Particle-particle friction coeff. & - \\
\texttt{FRICTION\_COEFF\_WALL} & Wall-particle friction coeff. & - \\
\texttt{ROLLING\_FRICTION\_COEFF\_PARTICLE} & Particle-particle rolling friction coeff. & - \\
\texttt{ROLLING\_FRICTION\_COEFF\_WALL} & Wall-particle rolling friction coeff. & - \\
\texttt{PARTICLE\_RADIUS\_LARGE} & Large particle radius & m \\
\texttt{PARTICLE\_RADIUS\_SMALL} & Small particle radius & m \\
\texttt{CONTAINER\_WIDTH} & Container width & m \\
\texttt{CONTAINER\_HEIGHT} & Container height & m \\
\texttt{PARTICLE\_GEN\_LAYERS} & Particle generation layers & - \\
\texttt{RANDOM\_SEED} & Random seed & - \\
\texttt{OUTPUT\_INTERVAL} & Output interval & steps \\
\texttt{ENABLE\_COULOMB\_FORCE} & Enable Coulomb force & 0/1 \\
\texttt{COULOMB\_CUTOFF} & Coulomb force cutoff radius & m \\
\texttt{ENABLE\_WALL\_WITHDRAW} & Enable wall withdrawal & 0/1 \\
\texttt{WALL\_WITHDRAW\_STEP} & Wall withdrawal start step & steps \\
\bottomrule
\end{tabularx}
\end{table}
...
Fig.~\ref{fig:flowchart}にDEMプログラムの全体フローチャートを示す。
